...
const Application = (function (UICtrl, APICtrl, GlobalCtrl, SpecialCtrl, WebSocketCtrl) {
  ...
  const loadInit = function () {
    ...
    const constructMessageOthers = (mine, time, received, id) => {
      ...
      holderSpanx.onclick = (e) => { GlobalCtrl.showGroupEllipses(e); return false }
    ...
    const loadMessages = async (number) => {
      ...
      const messageURL = window.location.protocol + '//' + window.location.host + `/api/messages/list-through/${group.name}/${number}/latest/`

      messages = await APICtrl.postAPI_Json(messageURL, { token: token })
      ...
      if (messages.length == 0) {
        ...
        messageContext = document.createTextNode('There are no messages in this group yet!')
          ...
          if (message.message_type === 'Text') {
            ...
            switch (message.message) {
              ...
              case "Dialogue with":
                ...
                messageContext = document.createTextNode(message.message + ` ${group.name}`)
            ...
            if (message.sender == chattyUser.id) {
              ...
              if (message.date_received == undefined) {
                ...
                if (message.message_refer == undefined) {
                  ...
                  await WebSocketCtrl.sendToUserID(message.sender, {
                    to: message.sender,
                    toGroup: group.name,
                  ...
                  const refx = await APICtrl.postAPI_Json(url, { token: token })

                  if (refx.sender == chattyUser.id) {
    ...
    const updateMessage = async (id) => {
      ...
      const messageURL = window.location.protocol + '//' + window.location.host + `/api/messages/by-id/${id}/`

      const message = await APICtrl.postAPI_Json(messageURL, { token: token })
    ...
    const addMessage = async (id) => {
      ...
      const messageURL = window.location.protocol + '//' + window.location.host + `/api/messages/list-through/${group.name}/10/latest/`
    ...
    const webSocketsHandler = async () => {
      ...
      chatSocket.onmessage = async (msg) => {
        ...
        if (data.to == chattyUser.username && data.toGroup == group.name && data.type == 'message') {

          loadMessages(50)

        } else if (data.to == chattyUser.username && data.toGroup == group.name && data.type == 'add-message') {

          addMessage()

        } else if (data.to == chattyUser.username && data.toGroup == group.name && data.type == 'update-message') {

          updateMessage(data.id)

        } else if (data.to == chattyUser.username && data.toGroup == group.name && data.type == 'delete-message') {
